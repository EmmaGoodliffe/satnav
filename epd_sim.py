...
def to_bit_sequence(x: int):
    """e.g. `12` -> `[1, 1, 0, 0]`"""
    w = 1 << np.arange(int(np.log2(x)), -1, -1)  # e.g. 4, 2, 1
    return np.array((x & w) > 0, dtype=np.int64)


def to_bit_sequences(l: int, xs: Ints):
    """e.g. `[3, 1]` -> `[[1, 1], [0, 1]]` when `l = 2` ∵ `l` is length of each bit sequence"""
    if len(xs.shape) != 1:
        raise Exception(f"xs are supposed to be 1D; received shape {xs.shape}")
    w = 1 << np.arange(l - 1, -1, -1)  # e.g. 4, 2, 1
    return np.array((xs.reshape(len(xs), 1) & w) > 0, dtype=np.int64)
...
class Epd_Sim:
    ...
    def display(self, ax: Axes):
        ...
        # bit_pairs = bit_pairs[:, ::-1]  # TODO: reverse pairs for actual display
        ...
        cbar = unwrap(ax.get_figure()).colorbar(mesh)
